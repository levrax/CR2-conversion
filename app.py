...
ЗАЧЕМ ОТДЕЛЬНЫЙ ФАЙЛ.  PyInstaller умеет взять .pyw напрямую, но модуль с
расширением .pyw нельзя импортировать обычным `import cr2_gui`: importlib
признаёт только .py/.pyc/.pyd.  Поэтому здесь два пути загрузки:

* `import cr2_gui` — работает в собранном приложении (cr2app.spec кладёт в
  сборку копию cr2_gui.py, см. комментарий в спеке) и сработал бы, если файл
  когда-нибудь переименуют в cr2_gui.py;
* загрузка cr2_gui.pyw через importlib — обычный запуск из папки с исходниками.
